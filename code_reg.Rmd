...
*Is it wise to use FDR as a selection tool for building good sets of candidate models? If not, then why? What kind of p-values do we get? Describe a scenario where using FDR would not yield good results.*
...
*You are given a significant heterogeneous dataset.*/
*How would you choose a sample subset of this dataset to train a model on it?*

*I.How would you choose a sample subset of this dataset to train a model on it?*
...
*II.For this assignment, split the data into test and training sets randomly such that only 15% of the records are in the training set. Then fit a simple linear regression model to predict the overall score of a player and test your model against the test set.*
...
*What are stepwise forward regression procedures? How are they better than adopting null or full models?*
...
*What are the benefits of using Lasso Regression?*
...
*Using the training data from question 2, fit a Lasso regression to predict the overall scores of players in the test set. Use the default value of alpha (alpha used to tune the penalty parameter), which is usually 1.*
